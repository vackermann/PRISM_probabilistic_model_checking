...
module scheduler
	...
	// A sends 0st bit of nth secret (for  n=1..N-1), B is next
 	[receiveB] phase=2 & party=1 & N>1 & n=0 -> (party'=2);
	// B sends bth bit of nth secret (for  n=1..N-1), move to next secret
	[receiveA] phase=2 & party=2 & N>1 & n<N-1 -> (n'=n+1);
	// B sends bth bit of Nth secret (for  n=1..N-1), A will go next with b=1
	[receiveA] phase=2 & party=2 & N>1 & n=N-1 -> (party'=1) & (n'=1);
	// A sends bth bit of nth secret, (for  n=2..N-1) move on to next secret
	[receiveB] phase=2 & party=1 & N>1 & n>0 & n<N-1-> (n'=n+1);
	// A sends bth bit on Nth secret (for  n=1..N-1), increment b and reset n
	[receiveB] phase=2 & party=1 & N>1 & n=N-1 & b<L -> (n'= 0) & (b'=b+1);
	// A sends (final) Lth bit of Nth secret (for  n=1..N-1), reset n and b and move to next phase
	[receiveB] phase=2 & party=1 & N>1 & n=N-1 & b=L -> (n'= 0) & (b'=1) & (phase'=3);
	
	// A sends 0st bit of (N+n)th secret (for  n=1..N-1), B is next
 	[receiveB] phase=3 & party=1 & N>1 & n=0 -> (party'=2);
	// B sends bth bit of (N+n)th secret (for  n=1..N-1), move to next secret
	[receiveA] phase=3 & party=2 & N>1 & n<N-1 -> (n'=n+1);
	// B sends bth bit of 2Nth secret (for  n=1..N-1), A will go next with b=1
	[receiveA] phase=3 & party=2 & N>1 & n=N-1 -> (party'=1) & (n'=1);
	// A sends bth bit of (N+n)th secret, (for  n=2..N-1) move on to next secret
	[receiveB] phase=3 & party=1 & N>1 & n>0 & n<N-1-> (n'=n+1);
	// A sends bth bit on 2Nth secret (for  n=1..N-1), increment b and reset n
	[receiveB] phase=3 & party=1 & N>1 & n=N-1 & b<L -> (n'= 0) & (b'=b+1);
	// A sends (final) Lth bit of 2Nth secret (for  n=1..N-1), reset n and b and move to next phase
	[receiveB] phase=3 & party=1 & N>1 & n=N-1 & b=L -> (phase'=4);

	// Special case: N=1
	// A sends first bit, then B, then for next bit b
	[receiveB] phase=2 & party=1 & n=0 & N=1 -> (party'=2);
	[receiveA] phase=2 & party=2 & n=0 & N=1&  b<L-> (party'=1) & (b'=b+1);
	// until B has reached final Lth bit, then repeat the same for phase 3
	[receiveA] phase=2 & party=2 & n=0 & N=1 & b=L-> (party'=1) & (b'=1) & (phase'=3);

	// A sends first bit, then B, then for next bit b
	[receiveB] phase=3 & party=1 & n=0 & N=1 -> (party'=2);
	[receiveA] phase=3 & party=2 & n=0 & N=1&  b<L-> (party'=1) & (b'=b+1);
	// until B has reached final Lth bit, then repeat the same for phase 3
	[receiveA] phase=3 & party=2 & n=0 & N=1 & b=L-> (phase'=4);
