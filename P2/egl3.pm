...
module scheduler
	...
	// A sends bth bit of nth secret (for n=1..N-1), B will go next
	[receiveB] phase=2 & party=1 -> (party'=2) ;
	// B sends bth bit of nth secret (for n=1..N-1), A will go next with next secret
	[receiveA] phase=2 & party=2 & n<N-1 -> (party'=1) & (n'=n+1);
	// B sends bth bit of Nth secret, increment b, reset n->1 and go back to A
	[receiveA] phase=2 & party=2 & n=N-1 & b<L-> (party'=1) & (n'=0) & (b'=b+1);
	// B sends final (Lth) bit of last Nth secret, move to next phase (N+1..2N) and back to A
	[receiveA] phase=2 & party=2 & n=N-1 & b=L -> (phase'=3) & (party'=1) & (n'=0) & (b'=1);
	// A sends bth bit of (N+n)th secret (for n=1..N-1), B will go next
	[receiveB] phase=3 & party=1 -> (party'=2) ;
	// B sends bth bit of (N+n)th secret (for n=1..N-1), A will go next with next secret
	[receiveA] phase=3 & party=2 & n<N-1 -> (party'=1) & (n'=n+1);
	// B sends bth bit of 2Nth secret, increment b, reset n->(N+1) and go back to A
